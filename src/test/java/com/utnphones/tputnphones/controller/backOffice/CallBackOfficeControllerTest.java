...
class CallBackOfficeControllerTest extends AbstractMVCTest {
    ...
    when(callService.getCallByUserAndRank(dni,start,end,getPageable())).thenReturn(getCallPage());

    ResponseEntity<List<Call>> responseEntity = callBackOfficeController.getCallByUser(auth,dni,start,end,getPageable());
